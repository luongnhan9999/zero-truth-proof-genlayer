...
export default function App() {
  ...
  const fetchTasksFromContract = async () => {
    try {
      ...
      if (Array.isArray(parsedTasks)) {
        ...
        addLog(`[CONTRACT] Read complete. Found ${parsedTasks.length} real on-chain tasks.`);
      ...
      console.error(err);
      addLog(`[CONTRACT READ ERROR] Read get_all_tasks failed: ${err.message || err}`);
      setTasks([]);
      setSelectedTaskId('');
    } finally {
      setIsLoading(false);
  ...
  const handleResolveEscalation = async () => {
    ...
    try {
      ...
      try {
        ...
        addLog(`[CHAIN CONFIRMED] Escalation resolved on-chain. Hash: ${hash}`);
      } catch (e) {
        addLog(`[CHAIN WARNING] Escalation transaction sent. Hash: ${hash}`);
      }
      await new Promise(r => setTimeout(r, 2000));
      await fetchTasksFromContract();
    } catch (err: any) {
  ...
  const handleResolveDisputeConsensus = async () => {
    ...
    try {
      setIsLoading(true);
      addLog(`[CHAIN TX] Invoking resolve_dispute_consensus on-chain for task ${activeTask.id}...`);
      ...
      try {
        ...
        addLog(`[CHAIN CONFIRMED] Multi-validator dispute consensus completed on-chain. Hash: ${hash}`);
      } catch (e) {
        addLog(`[CHAIN WARNING] Consensus arbitration transaction broadcasted. Hash: ${hash}`);
      }
      await new Promise(r => setTimeout(r, 2000));
      await fetchTasksFromContract();
    } catch (err: any) {
